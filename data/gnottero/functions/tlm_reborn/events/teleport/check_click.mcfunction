#> Check which player has clicked on the slot and teleport him to the last millenium
    ...
    #> Call the "gnottero:tlm_reborn/events/teleport/enter_tlm" function if the player isn't in The Last Millenium
        execute if entity @s[tag=!global.ignore,tag=!global.ignore.pos,scores={gn.has_confirmed=1..},predicate=gnottero:tlm_reborn/can_enter] run function gnottero:tlm_reborn/events/teleport/enter_tlm

    #> Call the "gnottero:tlm_reborn/events/teleport/enter_tlm" function if the player is in The Last Millenium
        execute if entity @s[tag=!global.ignore,tag=!global.ignore.pos,scores={gn.has_confirmed=1..},predicate=gnottero:tlm_reborn/can_exit] run function gnottero:tlm_reborn/events/teleport/exit_tlm
