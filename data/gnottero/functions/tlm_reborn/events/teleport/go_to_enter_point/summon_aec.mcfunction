#> Summon an area effect cloud at the position of the player that is leaving The Last Millenium
    ...
    #> Executing as the aec the "gnottero:tlm_reborn/events/teleport/go_to_enter_point/copy_pos" function
        execute as @e[type=area_effect_cloud,limit=1,sort=nearest,tag=gn.leave_tlm] at @s run function gnottero:tlm_reborn/events/teleport/go_to_enter_point/copy_pos